...
/**
 * @ClassName : CostTimeLoggerInterceptor  //类名
 * @Description : aop拦截器  //描述
 * @Author : JiangWenJie  //作者
 * @Date: 2019-12-04 19:30  //时间
 */
@Component
@Aspect
public class CostTimeLoggerInterceptor {
    private static final Logger logger = LoggerFactory.getLogger(CostTimeLoggerInterceptor.class);

    @Around("@annotation(com.jwj.im.aop.CostTimeLogger)")
    public Object costLogger(ProceedingJoinPoint pjp) throws Throwable {
        ...
        CostTimeLogger costLogger = null;
        try {
            //获取当前方法
            Method currentMethod = target.getClass().getMethod(msig.getName(), msig.getParameterTypes());
            //获取costLogger注解
            costLogger = currentMethod.getAnnotation(CostTimeLogger.class);
            //当有costLogger注解时
            if (costLogger != null) {
                //开始时间
